...
import CardProductThumbnail from "@/components/products/create/CardProductThumbnail"; // new import
...
import CardProductLicenses from "../create/CardProductLicenses";
  ...
  return (
    <form className="space-y-4 max-w-xl mx-auto p-4" onSubmit={handleSubmit}>
      ...
      <CardProductThumbnail
        thumbnailUrl={""} // optional initial URL for editing
        onChange={(file) => setThumbnailFile(file)} // set the file in parent state
      />
      ...
      {/* Content URL */}
      <CardProductVersion value={version} onChange={setVersion} onVersion={setIsVersionChecked}/>

      {/*<Card>
        <CardHeader>
          <CardTitle>Download Link (source)</CardTitle>
        </CardHeader>
        <CardContent>
          <input
            type="text"
            placeholder="Source Download Link"
            value={srcDownloadLink}
            onChange={(e) => setProductDownloadLink(e.target.value)}
