...
import { validateImageFile } from "@/lib/validate/validateImageFile"; // Your image validation function
import ErrorAlert from "@/components/banners/ErrorAlert"; // ErrorAlert component
...
interface Props {
  ...
  maxSize?: number; // maximum image size in bytes
}

export default function CardProductThumbnail({
  thumbnailUrl,
  onChange,
  allowedTypes = ["image/jpeg", "image/png", "image/webp"], // default types
  maxSize = Infinity, // default size limit (no limit)
}: Props) {
  ...
  const handleFileSelect = (file: File) => {
    try {
      validateImageFile(file, allowedTypes, maxSize);
      setLocalFile(file);
      onChange(file);
      setError(null); // Clear error if validation passes
    } catch (err: any) {
      setLocalFile(null); // Reset the file if invalid
      onChange(null); // Reset parent value
      setError(err.message); // Set validation error
    }
  };
  ...
  const removeFile = () => {
    ...
    setError(null); // Clear error on removal
  };
    ...
    <section id="product-thumbnail">
      <Card>
        ...
        <CardContent className="flex flex-col items-center gap-4">
          ...
          />
          {error && <ErrorAlert message={<span dangerouslySetInnerHTML={{ __html: error }} />} />}
