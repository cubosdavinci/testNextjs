...
export default function EditLicenses({ productId, licenses: initialLicenses }: EditLicensesProps) {
  const [licenses, setLicenses] = useState<ProductLicense[]>(initialLicenses ?? []);
  ...
  // Fetch licenses if not passed
  useEffect(() => {
    if (initialLicenses) {
      if (initialLicenses.length > 0) {
        setMainLicenseId(initialLicenses.find(l => l.is_main)?.id || null);
      }
      return;
    }
    ...
    const fetchLicenses = async () => {
      ...
      try {
        ...
        if (!data?.data || data.data.length === 0) {
          ...
          setMainLicenseId(data.data.find((l: ProductLicense) => l.is_main)?.id || null);
        }
      } catch (err: any) {
  ...
  const handleLicenseChange = (id: string, field: keyof ProductLicense, value: any) => {
    setLicenses(prev =>
      prev.map(l => (l.id === id ? { ...l, [field]: value } : l))
    );
  };

 const handleSave = async (license: ProductLicense) => {
  setSavingId(license.id);
  try {
    // Convert price in USD to cents
    const price_cents = license.price_cents;
            ...
            <tr key={lic.id} className="border-t">
              <td className="px-4 py-2">
                <input
                  ...
                  onChange={(e) => handleLicenseChange(lic.id, "license_name", e.target.value)}
              ...
              <td className="px-4 py-2">
                <input
                  type="number"
                  value={lic.price_cents / 100}
                  min={0}
                  step={0.01}
                  onChange={(e) =>
                    handleLicenseChange(lic.id, "price_cents", Math.round(parseFloat(e.target.value) * 100))
                  }
                  className="border p-1 rounded w-full"
                />
              </td>
