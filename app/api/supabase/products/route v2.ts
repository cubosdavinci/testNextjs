...
//import { getProductsByCategory } from "@/lib/db/categories/getCategoryTreeByProduct";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    async function executeOperation(name: string, vars: any, thumbnail?: File | null, gallery?: File | null) {
      switch (name) {
        case "createProduct":
          return createProduct( vars, thumbnail, gallery);
        case "getProduct":            
    ...
    return NextResponse.json(result);
      ...
      consoleLog("💥 Route Exception - api/products: ", err.message);    
      
      let errorMessage = "❌ Server error. Please try again later.";
      let status = 400;

      if (err instanceof GoogleApiError) {
        errorMessage = err.message; // show the Google API error message
        status = err.status || 400;
      } else if (err instanceof Error) {
        errorMessage = err.message; // fallback for other JS errors
      }
