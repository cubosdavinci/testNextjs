...
export default function SolanaPage() {
  ...
  const [sending, setSending] = useState(false); // <-- button lock

  // USDC mint on devnet
  const USDC_MINT = new PublicKey("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");

  // Detect network/cluster
  useEffect(() => {
  ...
  // Generate nonce for each transaction
  const createNonce = () => uuidv4();

  const sendSOL = async () => {
    if (sending) return; // prevent multiple clicks
    setSending(true);
    const nonce = createNonce(); // unique id per click
    try {
      setError(null);
      setTxResult(null);
    ...
    <div className="p-6 space-y-4">
      ...
      {txResult && (
        <div className="mt-4 p-4 border rounded bg-gray-50 space-y-2">
          ...
          <p>Nonce: <strong>{txResult.nonce}</strong></p>
